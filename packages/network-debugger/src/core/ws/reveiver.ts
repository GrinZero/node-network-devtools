...
export class Receiver extends Writable {
  ...
  decompress(data: Buffer, cb: (err?: Error | null) => void): void {
    const perMessageDeflate = this._extensions[PerMessageDeflate.extensionName] as PerMessageDeflate
  ...
  createError(ErrorCtor: any, message: any, prefix: any, statusCode: any, errorCode: any) {
    this._loop = false
    this._errored = true
