...
function waitForMessage(child, messages, predicate, timeoutMs = IO_TIMEOUT_MS) {
  ...
  if (child.exitCode !== null || child.signalCode !== null) {
    return Promise.reject(new Error(`Legacy consumer already exited with ${child.exitCode}`))
  }
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error('Timed out waiting for Legacy consumer IPC'))
    }, timeoutMs)
    const cleanup = () => {
      clearTimeout(timer)
    ...
    const onExit = (code, signal) => {
      cleanup()
      reject(new Error(`Legacy consumer exited before IPC (code=${code}, signal=${signal})`))
    }
...
function waitForExit(child, timeoutMs = IO_TIMEOUT_MS) {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      child.off('exit', onExit)
      reject(new Error('Timed out waiting for Legacy consumer to exit'))
    }, timeoutMs)
    const onExit = () => {
      clearTimeout(timer)
      resolve()
...
function runFinitePreload(packageDirectory, timeoutMs = 15_000) {
  const registerUrl = pathToFileURL(join(packageDirectory, 'dist/register.mjs')).href
  const source =
    "process.stdout.write('@@NND_PACK_FINITE@@' + JSON.stringify({ pid: process.pid }) + '\\n')"
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [`--import=${registerUrl}`, '-e', source], {
    ...
    child.once('close', (code, signal) => {
      ...
      if (timedOut) {
        reject(
          new Error(
            `Packed finite preload did not exit naturally on ${process.version}.\n` +
              `stdout:\n${stdout}\nstderr:\n${stderr}`
          )
        )
        return
...
async function waitForEndpointClosed(url, timeoutMs = 5_000) {
  ...
  while (Date.now() < deadline) {
    try {
      await requestBuffer(url, 500)
    } catch {
      return
    }
    await new Promise((resolve) => setTimeout(resolve, 25))
  }
  throw new Error(`Packed finite Legacy target remained reachable after exit: ${url}`)
}

test(
  'the packed Legacy adapter runs a real CDP lifecycle under this Node runtime',
  { timeout: TEST_TIMEOUT_MS },
  async () => {
    const installation = await installPackedPackage('nnd-legacy-runtime-')
    const consumerRequire = createRequire(join(installation.root, 'consumer.cjs'))
    const WebSocket = consumerRequire('ws')
    const fixture = join(installation.root, 'legacy-runtime-consumer.cjs')
    await copyFile(join(FIXTURE_DIRECTORY, 'legacy-runtime-consumer.cjs'), fixture)

    const token = `${process.platform}-${process.version}-${process.pid}`
    const expectedBody = Buffer.from(`legacy-pack-runtime:${token}`)
    ...
    try {
      ...
      await assert.rejects(
        ...
        'dispose must close the Legacy discovery endpoint before resolving'
      )

      const finite = await runFinitePreload(installation.packageDirectory)
      assert.equal(
        finite.code,
        0,
        `packed finite preload failed on ${process.version}:\n${finite.stderr}`
      )
      assert.equal(finite.signal, null)
      assert.doesNotMatch(finite.stderr, /NND_LEGACY_CHILD_START_FAILED/)
      assert.equal(finite.stdout.match(/@@NND_PACK_FINITE@@/g)?.length, 1)
      const readyPrefix = '[nnd:ready] '
      const readyLines = finite.stderr.split(/\r?\n/).filter((line) => line.includes(readyPrefix))
      assert.equal(readyLines.length, 1, finite.stderr)
      const finiteReady = JSON.parse(
        readyLines[0].slice(readyLines[0].indexOf(readyPrefix) + readyPrefix.length)
      )
      assert.equal(finiteReady.mode, 'legacy')
      assert.match(finiteReady.target.discoveryUrl, /^http:\/\/127\.0\.0\.1:\d+\/json\/list$/)
      await waitForEndpointClosed(finiteReady.target.discoveryUrl)
    } finally {
